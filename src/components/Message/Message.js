...
const Message = ({ image, title, content, time, id, unread }) => {
	...
	return (
		<div className="message_container" onClick={() => (window.location = `/messages/details/${id}`)}>
			...
			<div className="message_container__content">
				...
				<div className={`${"message_container__content__message"} ${unread && unread > 0 && unreadMessages}`}>
					{content}
				</div>
			</div>
			<div className={"message_container__time"}>{`${new Date(time).getHours()}:${new Date(time).getMinutes()}`}</div>
		</div>
	);
};
