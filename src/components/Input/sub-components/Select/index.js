...
class Select extends Component {
    ...
    if (this.wrapperRef && !this.wrapperRef.current.contains(event.target)) {
      this.setState((state) => {
        state.externalSource = this.props.multiple
          ? this.props.externalSource.map((el) => {
              el.selected = this.state.selectedValues.some(
                (vl) => vl.key === el.key
              );
              return el;
            })
          : this.props.externalSource.map((el) => {
              el.selected = this.state.selectedValue.key === el.key;
              return el;
            });
  ...
  onChange = (event) => {
    ...
    if (event.target.value) {
      let tempSource = this.props.externalSource.filter((el) =>
        el.value.toLowerCase().includes(event.target.value.toLowerCase())
      );
      if (multiple)
        tempSource.map((el) => {
          el.selected = this.state.selectedValues.some(
  ...
  onClickListItem = (key) => {
    ...
    if (multiple) {
      let tempSource = this.state.externalSource.map((el) => {
        el.selected = key === el.key ? !el.selected : el.selected;
        return el;
      });
      let tempSelectedItems = tempSource.filter((el) => el.selected);
      this.setState({
        externalSource: [...tempSource],
        selectedValues: tempSelectedItems,
      });
