export default {
  checkValidation(value = '', validations, errorList) {
    ...
    for (let key in validations) {
      if (key === 'customValidations') {
        ...
        errorList.find((e) => e.key === key).valid = this[key](
          value,
          validations[key]
        );
  ...
  shouldHaveString(
    value = '',
    { whatToHave = [], shouldHave = true, canEmpty = false }
  ) {
  ...
  shouldHaveNumberWithCount(value = '', { shouldHave = true }) {
    return !(/\d/.test(value) ^ shouldHave);
  },
  ...
  lengthValidator(
    value,
    { start = 0, stop = value?.toString().length, canEmpty = false }
  ) {
    if (!value?.toString().length && canEmpty) {
      return true;
    }
  ...
  sWith(
    value,
    { endsWith = false, whatToHave = '', shouldHave = true, canEmpty = false }
  ) {
    value = value.toLowerCase();
    if (!value.length && canEmpty) return true;
    if (typeof whatToHave != 'string') {
      for (let val of whatToHave) {
        if (
          !(
            (endsWith ? value.endsWith(val) : value.startsWith(val)) ===
            shouldHave
          )
        ) {
          return false;
    ...
    return (
      (endsWith ? value.endsWith(whatToHave) : value.startsWith(whatToHave)) ===
      shouldHave
    );
  },
};
