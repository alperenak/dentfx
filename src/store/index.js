...
export default {
  async createUser({name, surname, email, password, phone, birthDate, city, country, loginType}) {
    let baseUrl = config.baseUrl;
    let tokenCookieName = "token";
    let path = `/user`;
    let payload = {name, surname, email, password, phone, birthDate, city, country, loginType};

    return await http.makePostRequest(path, baseUrl, tokenCookieName, payload, errorMessageBuilder);
  },
  async createClinic({name, email, password, phone, city, country, address, coordinate, longitude, latitude, treatmentType}) {
    let baseUrl = config.baseUrl;
    let tokenCookieName = "token";
    let path = `/clinic`;
    let payload = {name, email, password, phone, city, country, address, coordinate, longitude, latitude, treatmentType};

    return await http.makePostRequest(path, baseUrl, tokenCookieName, payload, errorMessageBuilder);
  },
  async createDentist({name, surname, email, password, phone, clinic, treatmentType}) {
    let baseUrl = config.baseUrl;
    let tokenCookieName = "token";
    let path = `/dentist`;
    let payload = {name, surname, email, password, phone, clinic, treatmentType};
  ...
  async createUserAppointment({user, clinic, dentist, treatmentType, startTime, endTime, isCheckIn, paymentType}) {
    let baseUrl = config.baseUrl;
    let tokenCookieName = "token";
    let path = `/dentist`;
    let payload = {user, clinic, dentist, treatmentType, startTime, endTime, isCheckIn, paymentType};
    return await http.makePostRequest(path, baseUrl, tokenCookieName, payload, errorMessageBuilder);
  },
  ...
  async updateUserProfile({userId, name, surname, email, phone, country, city}) {
    let baseUrl = config.baseUrl;
    let tokenCookieName = "token";
